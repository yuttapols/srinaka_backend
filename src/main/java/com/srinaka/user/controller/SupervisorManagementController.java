...
public class SupervisorManagementController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<UserResponse>> create(@Valid @RequestBody CreateSupervisorRequest request) {
        UserResponse response = userRegistrationService.createSupervisor(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success(response, "Supervisor has been created successfully."));
    }

    @GetMapping
    public ApiResponse<List<UserResponse>> list() {
        return ApiResponse.success(userRegistrationService.listByRole(UserRole.SUPERVISOR));
    }
}
